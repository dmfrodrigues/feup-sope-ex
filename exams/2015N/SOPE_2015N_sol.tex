...
When $t_f < t_p$, the consumer can consume at a higher rate than the producer can produce, so it will be blocked for $\Delta = t_p-t_f$ time units in \texttt{sem\_wait(\&sem)}; thus the consumer takes $t_f + 9(t_p+\Delta)+t_p$ (initially waits $t_f$, then consumes each of the items in $t_p$ and waits $\Delta$ for the next one, and then consumes the last one in $t_p$), which amounts to $t_f+9t_f + t_p = 10t_f + t_p$.
